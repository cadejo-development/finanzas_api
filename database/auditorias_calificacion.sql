...
-- 5. Eliminar duplicados / criterios anteriores de las 5 secciones oficiales
--    (hace el script idempotente sin depender de ON CONFLICT)
...
-- 6. Índice único (por si no existe; ahora la tabla ya está limpia)
CREATE UNIQUE INDEX IF NOT EXISTS aud_criterios_cat_nombre_uidx
    ON auditoria_criterios (categoria, nombre);

-- 7. Insertar los 39 criterios del formato oficial
