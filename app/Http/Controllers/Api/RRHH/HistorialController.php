        ...
        if (!$tipo || $tipo === 'vacacion') {
            ...
            $q->get()->each(function ($v) use (&$eventos) {
                $eventos->push([
                    ...
                    'creado_por'    => $v->creado_por ?? (explode('@', $v->aud_usuario ?? '')[0] ?: null),
        ...
        if (!$tipo || $tipo === 'incapacidad') {
            ...
            $q->get()->each(function ($i) use (&$eventos) {
                $eventos->push([
                    ...
                    'creado_por'    => $i->creado_por ?? (explode('@', $i->aud_usuario ?? '')[0] ?: null),
                    'aprobador_id'  => null,
                    'veredicto'     => null,
                    'decision_at'   => null,
                ]);
        ...
        if (!$tipo || $tipo === 'amonestacion') {
            ...
            $q->get()->each(function ($a) use (&$eventos) {
                $eventos->push([
                    ...
                    'creado_por'   => $a->creado_por ?? (explode('@', $a->aud_usuario ?? '')[0] ?: null),
                    'aprobador_id' => null,
                    'veredicto'    => null,
                    'decision_at'  => null,
                ]);
        ...
        if (!$tipo || $tipo === 'traslado') {
            ...
            $q->get()->each(function ($t) use (&$eventos) {
                $eventos->push([
                    ...
                    'creado_por'   => $t->creado_por ?? (explode('@', $t->aud_usuario ?? '')[0] ?: null),
        ...
        if (!$tipo || $tipo === 'desvinculacion') {
            ...
            $q->get()->each(function ($d) use (&$eventos) {
                $eventos->push([
                    ...
                    'creado_por'   => $d->creado_por ?? (explode('@', $d->aud_usuario ?? '')[0] ?: null),
        ...
        if (!$tipo || $tipo === 'cambio_salarial') {
            ...
            $q->get()->each(function ($c) use (&$eventos) {
                $eventos->push([
                    ...
                    'creado_por'   => $c->creado_por ?? (explode('@', $c->aud_usuario ?? '')[0] ?: null),
