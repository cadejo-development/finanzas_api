...
/**
 * Catálogos de ingredientes obtenidos desde Brilo (SQL Server) y materias primas locales.
 */
class BrewCatalogosController extends Controller
{
    ...
    // ─── Cervezas (catálogo de productos Cadejo en Brilo) ────────────────────
    ...
    // ─── Materias primas generales (de nuestra base PostgreSQL) ─────────────
    ...
    // ─── Helper: ejecutar query en SQL Server, con fallback ──────────────────
    private function queryBrilo(string $sql): array
    {
        try {
            ...
            // SQL Server no disponible — retornar catálogo vacío
            return [];
        }
    }
}
...
/**
 * Catálogos de ingredientes obtenidos desde Brilo (SQL Server) y materias primas locales.
 * Todos los métodos usan caché de 12h para minimizar round-trips al SQL Server.
 */
class BrewCatalogosController extends Controller
{
    private const TTL = 43200; // 12 horas en segundos

    // ─── Maltas / Granos ─────────────────────────────────────────────────────
    // GET /api/compras/brew/catalogos/maltas
    public function maltas()
    {
        $data = Cache::remember('brew_cat_maltas', self::TTL, function () {
            return $this->queryBrilo(
                "SELECT LTRIM(RTRIM(proCodigo)) AS codigo,
                        LTRIM(RTRIM(proNombre))  AS nombre
                 FROM   olComun.dbo.Productos WITH (NOLOCK)
                 WHERE  proActivo = 1
    ...
    public function lupulos()
    {
        $data = Cache::remember('brew_cat_lupulos', self::TTL, function () {
            return $this->queryBrilo(
                "SELECT LTRIM(RTRIM(proCodigo)) AS codigo,
                        LTRIM(RTRIM(proNombre))  AS nombre
                 FROM   olComun.dbo.Productos WITH (NOLOCK)
    ...
    public function cervezas()
    {
        $data = Cache::remember('brew_cat_cervezas', self::TTL, function () {
            return $this->queryBrilo(
                "SELECT LTRIM(RTRIM(proCodigo)) AS codigo,
                        LTRIM(RTRIM(proNombre))  AS nombre
                 FROM   olComun.dbo.Productos WITH (NOLOCK)
    ...
    private function queryBrilo(string $sql): array
    {
        try {
            ...
            // Si no hay conexión a Brilo, retornar catálogo vacío (no romper la app)
            return [];
        }
    }
}
